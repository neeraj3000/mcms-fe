...
export async function registerCoordinator(name, mobileNo, email, password) {
  try {
    ...
    // Generate new userId by querying for the last user
    const lastUserQuery = query(usersRef, orderBy("userId", "desc"), limit(1));
    const lastUserSnapshot = await getDocs(lastUserQuery);
    ...
    // Use newUserId as the document ID for the user
    ...
    // Generate new coordinatorId by querying for the last coordinator
    ...
    // Use newCoordinatorId as the document ID for the coordinator
    const coordinatorDocRef = doc(coordinatorsRef, newCoordinatorId.toString());
    const coordinatorData = {
      coordinatorId: newCoordinatorId,
      name,
      mobileNo,
    ...
    await setDoc(coordinatorDocRef, coordinatorData);
